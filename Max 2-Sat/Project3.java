...
public class Project3
{
		...
		for (int i=0; i<variablesVal.length;i++)
		{
			for (int j=0; j<pairs.size(); j++)
			{
				if (Math.abs(pairs.get(j)) == variables.get(i))
					pairs.set(j,pairs.get(j) * variablesVal[i]);
			} 
		}
		return pairs;
	}

	public static int[] bruteForce(int[] variablesVal,ArrayList<Integer> pairs, ArrayList<Integer> variables)
	{
		...
		if (variablesVal[variablesVal.length-1] != 0)
		{
		...
		else
		{
			...
			//This for loop will determine where the next position is to assign a truth value.
			int counter=0;
			for (int i=0;i<variablesVal.length;i++)
			{
				if (variablesVal[i]==0)
			...
			int []firstSol= bruteForce(variablesVal, pairs, variables );
			variablesVal[counter]=-1;
			int []secondSol= bruteForce(variablesVal, pairs, variables );
			//Take the solution with the maximum amount of true clauses.
			if (firstSol[variablesVal.length] > secondSol[variablesVal.length])
				return firstSol;
			else
			{
				return secondSol;
	...
	public static void main(String[] args)
	{
		...
		//Setup environment for the call to a solution. An array is needed to pass into the function which will hold the set of truth values. Another array is needed to hold the actual solution returned by the method.
		...
		if (input==1)
		{
			actualsolution=bruteForce(variablesval, pairs, variables);
